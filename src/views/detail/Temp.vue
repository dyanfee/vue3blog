<template>
  <div class="wrapper">
    <div class="container">
      <div class="menu">
        <ul class="menu-list">
          <li v-for="(nav, index) in navList" :key="index" @click="currentClick(index)">
            <!-- :class="{ on: activeIndex === index }" -->
            <a href="javascript:0;" @click="pageJump(nav.index)">{{ nav.title }}</a>
            <div
            ...
            >
              <ul class="nav-list">
                ...
                >
                  ...
                  <a href="javascript:0;" @click="pageJump(item.index)"
                    >{{ item.title }}
                  </a>
                </li>
              </ul>
...
export default {
  ...
  created() {
    ...
    getArticle({ id }).then((res) => {
      this.mdContent = res.body.content;
  ...
  methods: {
    ...
    pageJump(id) {
      ...
      this.$el.querySelector(`#data-${id}`).offsetTop - 60;
